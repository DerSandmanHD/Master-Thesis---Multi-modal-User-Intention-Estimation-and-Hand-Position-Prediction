...
def run_audio_pipeline(vrs_file_path, proband_id, task_name):
    ...
    print("Extrahiere Audiospur aus der VRS-Datei...")
    num_audio_samples = provider.get_num_data(audio_stream_id)
    
    if num_audio_samples == 0:
        print("Fehler: Keine Audiodaten in dieser VRS-Datei gefunden!")
        return

    audio_data_list = []
    # Loop ueber alle Audiorohdaten im Stream
    for i in range(num_audio_samples):
        audio_tuple = provider.get_audio_data_by_index(audio_stream_id, i)
        # Ohne Klammern auslesen und direkt in ein numpy-Array konvertieren
    ...
    # Standard-Aria-Samplerate definieren und als WAV abspeichern
    sample_rate = 48000 
    wavfile.write(temp_audio_path, sample_rate, full_audio.astype(np.int16))
    print(f"Temporaere Audiodatei erstellt: {temp_audio_path}")

    print("Lade KI-Sprachmodell (OpenAI Whisper)...")
    model = whisper.load_model("base")

    print("Analysiere Sprachbefehle fuer die KI-Metadaten...")
    ...
    # JSON-Metadaten exportieren
    with open(json_filename, 'w', encoding='utf-8') as f:
        json.dump(metadata, f, indent=4, ensure_ascii=False)
        
    # Temporaere WAV-Datei entfernen
...
if __name__ == "__main__":
    # Pfad zur lokalen Test-Aufnahme definieren
    run_audio_pipeline(
