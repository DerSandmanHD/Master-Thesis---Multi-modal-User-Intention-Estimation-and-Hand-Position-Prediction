...
* **Koordinatentransformationen:** Hand, Gaze und Objekte werden mit den kalibrierten Extrinsiken und der SLAM-Pose aus dem Device- beziehungsweise Kamerasystem in Weltkoordinaten transformiert. AprilTag 0 definiert einen statischen Roboteranker: Aus allen Frames mit gleichzeitig gültigem Tag und SLAM wird nach Translation-Ausreißerfilterung eine robuste Weltpose geschätzt. Bei Verdeckung wird der Roboterframe über die aktuelle SLAM-Pose fortgeführt und durch `robot_frame_valid` sowie `robot_anchor_interpolated` gekennzeichnet. Der noch nicht vermessene physische Offset vom Marker zur realen Roboterbasis wird bewusst nicht angenommen.
* **Trainingsziele:** Jede Zeile erhält `continue`, `fetch`, `transition` oder `handover`. `transition` hält die Sensorzeitreihe zwischen `DONE` und `THIRD` kontinuierlich, ist aber kein ML-Target und darf nur als Fensterhistorie dienen. Zusätzlich werden zukünftige Wrist-Positionen und -Quaternionen beider Hände für einen standardmäßigen Vorhersagehorizont von 1 s erzeugt. Der Trainings-Loss verwendet das ausgewählte Empfangshand-Target ausschließlich während `handover`. Fehlende zukünftige Posen erhalten Validitätsmasken.
...
* **Historische Annahme, später bestätigt:** Dieser Zwischenstand behandelte `Test` als Pseudonym eines echten Teilnehmers und schloss den Namen nicht automatisch aus. Der aktive v3-Split-Audit markierte diese Herkunft zunächst zur manuellen Bestätigung; der Projektverfasser bestätigte sie am 26.08.2026. Der datierte [Provenienzvermerk](../Training/reports/dataset_v3_causal_20260815_n214_5d136a34/IDENTITY_PROVENANCE_RESOLUTION_20260826.md) hält die Auflösung fest. Nur unbekannte Teilnehmer (`Unknown_*` beziehungsweise `unknown_*`) und im manuellen Review explizit mit `decision=exclude` markierte Einzelaufnahmen erhalten `include_in_training=False`. Dadurch können unklare oder verworfene Aufnahmen im Arbeitsordner bleiben, ohne später versehentlich in das Training einzufließen.
...
| **2** | `"second"` | `"done"` | Proband fokussiert visuell ein Zielobjekt nahe des (simulierten) Roboterarms. | **fetch** | *Intention Alignment*: Verknüpfung von Gaze-Vektor und räumlicher Marker-ID zur Zielerkennung. |
...
Die explizit ausgeschlossenen Fälle `Test_4_20260527_161832`, `Unknown_2_20260616_141751` und `unknown_20260604_150319` müssen nicht für das Training korrigiert werden. Andere `Test`-Sequenzen wurden in diesem historischen Stand nach denselben QA-Regeln wie übrige Teilnehmer behandelt; am 26.08.2026 wurde bestätigt, dass der Name tatsächlich ein Teilnehmerpseudonym bezeichnet. Diese Identitätsbestätigung hebt die unabhängigen QA-Ausschlüsse einzelner Aufnahmen nicht auf. Da `next_action` priorisiert ist, können nach der Behebung der 17 MPS-Fälle weitere bislang verdeckte Timestamp-Probleme sichtbar werden. Die QA ist deshalb nach jedem größeren Verarbeitungsschritt erneut auszuführen.
...
Unter `Training/` liegt inzwischen eine reproduzierbare PyTorch-Trainingspipeline vor. Sie enthält einen fensterbasierten `Dataset`-/`DataLoader`, eine explizite Feature-Whitelist, Missing-Data-Masken, ausschließlich auf dem Trainingssplit geschätzte Normalisierung sowie personenbasierte Train-/Validation-/Test-Splits. `transition`-Zeilen duerfen als reale Historie in ein Fenster eingehen, aber nie dessen Target bilden; Fenster mit echten großen Timestamp-Sprüngen werden verworfen. Das Modell besitzt einen hierarchischen Kopf für `continue` gegen Assistenz, einen bedingten Kopf für `fetch` gegen `handover` und einen ausschließlich fuer Handover trainierten Kopf fuer die zukünftige Position und Orientierung der empfangenden Hand. Objektmarker bleiben Eingabekontext und werden nicht als Transformer-Zielklasse vorhergesagt.

Die aktuelle hierarchische Baseline wird über `Training/configs/models/transformer_v1.json`, `Training/smoke_test.py` und `Training/jobs/train_transformer_v1.sbatch` gesteuert. Checkpoints, verwendete Konfiguration, Datenmetadaten und Metriken werden unter `Training/runs/<dataset_tag>/<experiment_tag>/<model_tag>/<run_id>/` abgelegt. Der Testsplit wird erst nach Auswahl des besten Modells anhand der Validation-Macro-F1 ausgewertet. Der frühere Flat/Object-Lauf bleibt unter `archive/legacy_flat_object_baseline/` dokumentiert.
