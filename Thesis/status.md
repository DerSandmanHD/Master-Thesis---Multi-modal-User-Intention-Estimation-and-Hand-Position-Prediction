...
Die aktuelle hierarchische Baseline wird über `Training/configs/hierarchical_baseline_v1.json`, `Training/smoke_test.py` und `Training/hierarchical_baseline.sbatch` gesteuert. Checkpoints, verwendete Konfiguration, Datenmetadaten und Metriken werden unter `Training/runs/` abgelegt. Der Testsplit wird erst nach Auswahl des besten Modells anhand der Validation-Macro-F1 ausgewertet. Der frühere Flat/Object-Lauf bleibt unter `archive/legacy_flat_object_baseline/` dokumentiert.
