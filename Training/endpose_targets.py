...
All handover windows of an eligible sequence receive the same robust pose.
"""
    ...
    *,
    ...
    for endpoint_row in handover_rows[::-1]:
        ...
        candidate_mask = (
            ...
            & (timestamps <= endpoint_timestamp)
        )
        candidate_rows = np.flatnonzero(candidate_mask)
        valid_rows = candidate_rows[
            pose_valid[candidate_rows, hand_id]
            & (hand_ids[candidate_rows] == hand_id)
        ]
        valid_ratio = len(valid_rows) / max(1, len(candidate_rows))
        valid_span = (
            float((timestamps[valid_rows[-1]] - timestamps[valid_rows[0]]) / 1e9)
            if len(valid_rows) >= 2
            else 0.0
        )
        diagnostics = {
            "aggregation_start_timestamp_ns": (
                int(timestamps[candidate_rows[0]]) if len(candidate_rows) else None
            ),
