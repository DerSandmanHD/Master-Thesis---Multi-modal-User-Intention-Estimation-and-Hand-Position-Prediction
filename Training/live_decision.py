...
class InputQualityGate:
    ...
    def __init__(
        ...
        *,
        ...
        minimum_gaze_coverage: float = 0.80,
        maximum_gaze_gap_ms: float = 500.0,
        minimum_handover_hand_coverage: float = 0.50,
    ) -> None:
        ...
        self.frames: deque[tuple[int, bool, bool, bool, bool]] = deque(
    ...
    def assess(self, intention: str) -> dict:
        if intention not in INTENTIONS:
            raise ValueError(f"Unknown intention for quality gate: {intention}")

        frame_count = len(self.frames)
        ready = frame_count == self.window_size
        timestamps = [frame[0] for frame in self.frames]
        gaze = [frame[1] for frame in self.frames]
        left = [frame[2] for frame in self.frames]
        right = [frame[3] for frame in self.frames]
        robot_frame = [frame[4] for frame in self.frames]
        ...
        current_hand_valid = bool(left[-1] or right[-1]) if frame_count else False
        best_hand_coverage = max(left_coverage, right_coverage)
        if intention == "handover":
            if best_hand_coverage < self.minimum_handover_hand_coverage:
                reasons.append("handover_hand_coverage_too_low")
            if not current_hand_valid:
                reasons.append("handover_hand_missing_currently")
